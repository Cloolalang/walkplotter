...
- **Process → Dot size:** under the **View** zoom bar (when a floor plan is loaded), a **Dot size** slider (**1%–250%**) scales trail preview dots, RF markers, route stroke proportions, colour-coded ribbon width, and nudge hit targets on the **Process** tab only. Independent of Map **Pin size**. Persisted in **localStorage** (`walkplotter-process-dot-scale-v1`).
...
3. **Recent maps:** After you open an image from disk (Map **Choose map**, drag-and-drop on the map, or **Process → Map**), Walkplotter stores up to **eight** copies in the browser (**IndexedDB**, hashed by file content; max about **25 MB** per file—larger images are not kept). Under **Recent maps** on **Controls** and **Process**, tap a name to load that map again. **×** removes an entry from the list only (not from disk). Clearing site data removes recents. The bundled `floorimage.jpg` default is **not** added automatically.
...
6. **Optional — RSSI pre-process filtering tab:** open **RSSI pre-process filtering** to view a time-series graph of the currently loaded RSSI CSV and apply one filter mode at a time: **Rolling average** (window in samples, up to 5000) or **Lee criterion** (frequency + walking speed; window derived from spatial averaging length). Use **Save filtered RSSI CSV** to export `time,rssi` with the applied filter for manual re-load into Process/bundles.
...
9. **Map (after plotting):** Each matched sample is placed at trail coordinates. **Path loss:** **circles** / ribbon use the **interpolated** colour scale between editable **stops** from **−120** (weak) to **−30** (strong); out-of-range values **clamp**. **RSSI:** fixed **10 dBm** colour stops from **−120** to **−25 dBm** (clamped); histogram uses **10 dBm** bins over that span. **Original** samples: **grey ring**; **interpolated**: **no ring** (legend summarizes counts). When **both** path loss and RSSI overlays exist, use **Show → Path loss / RSSI** to switch map, legend, and histogram. Optional toggles (toolbar):
   ...
   - **Histogram** — **Path loss:** 20 dB bins down to **−120 dB** loss; **Range** column uses positive loss magnitude with **non-overlapping** integer endpoints between rows (e.g. **81–100** then **101–120**). **RSSI:** 10 dBm bins, signed **dBm** ranges in the table. **Count**, **Seconds** (sum of trail time to the next plotted point for samples in that bin), and **%**; first column is a **swatch** (mid-bin colour). **Pie chart** uses the same **%** per bin. RSSI histogram title also indicates whether data is **Raw** or **filtered** (rolling/Lee with parameters). For path loss only, expand **Bin & map colour scale** to edit stop colours (**localStorage**, key `walkplotter-process-pl-stops-v3`); **Reset to default** restores the built-in ramp. RSSI colours are fixed in the app (no per-stop editor in this version).
10. **Clear overlay** removes path loss and RSSI overlays and the histogram; it does not reload the CSV files from disk.
11. **Save process bundle** / **Load bundle:** After the required inputs are in place (**map** + **Walkplotter CSV** + at least one RF CSV), you can **Save process bundle**—a dialog lets you **choose the download file name** (default `walkplotter-process-bundle-…json`; `.json` is added if omitted). The file embeds the **map** (base64), full **Walkplotter CSV** (including nudges), **path-loss CSV** (if loaded), **RSSI CSV** (if loaded), and current Process view/settings (including overlay shift, map flip, dot size, heatmap settings + boundary polygon, Show metric/toggles, FSPL settings, RSSI offset). **Load bundle** restores all embedded data with a **single file pick**. Large images make **large** JSON files; the format is version **1** and may be extended later.
