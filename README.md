...
- **Process → Dot size:** under the **View** zoom bar (when a floor plan is loaded), a **Dot size** slider (**25%–250%**, same range as **Map** pin size) scales trail preview dots, RF markers, route stroke proportions, colour-coded ribbon width, and nudge hit targets on the **Process** tab only. Independent of Map **Pin size**. Persisted in **localStorage** (`walkplotter-process-dot-scale-v1`).
